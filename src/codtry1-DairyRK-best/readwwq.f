...
!!   phophorus changes 2/4/2015 gsm
!!    pstream_cod |              |instream phosphorus code adjustment option:
!!                               | 0	- no instream P
!!                               | 1	- call instream p subroutine
...
!! Phophorus changes 2/4/2015 gsm
      ...
      read (101,*,iostat=eof) nd_pflow
      if (eof < 0) exit
      read (101,*,iostat=eof) ptrans_in
      if (eof < 0) exit
      read (101,*,iostat=eof) ptrans_out
      if (eof < 0) exit
      read (101,*,iostat=eof) bfullp_eq
      if (eof < 0) exit
      read (101,*,iostat=eof) bfullp_mn
      if (eof < 0) exit
      read (101,*,iostat=eof) bfullp_mx
      if (eof < 0) exit
      read (101,*,iostat=eof) pdecay
      if (eof < 0) exit
      exit
      end do
...
!! phophorus changes 2/4/2015 gsm 
       !! Instream P Model MW 11/28/12
      if (nd_pflow <= 0.) nd_pflow = 200 
      if (ptrans_in <= 0.) ptrans_in = 0.05 
      if (ptrans_out <= 0.) ptrans_out = 0.01 
      if (bfullp_eq <= .0) bfullp_eq = 0.5 
      if (bfullp_mn <= -1.) bfullp_mn = -1. 
      if (bfullp_mx <= 0.) bfullp_mx = 0.7 
      if (pdecay <= 0.) pdecay = 0.01
...
!! phosphorus changes 2/4/2015 gsm  ievent ==3 instead of >0
!!      if (ievent > 0) then
      if (ievent == 3) then
        mumax = mumax / 24.
        rhoq = rhoq / 24.
