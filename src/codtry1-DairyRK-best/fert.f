...
!!  phosphorus changes 2/4/2015 gsm per mjw
     ...
     &      frt_kg * forgn(ifrt)
            ...
            !! MJW Manure P Layer
           if(manp_flag == 1 .and. l == 1 .and. forgp(ifrt) > 0. .and.  &
     &      ftype(ifrt) == 2) then
          ! add applied mineral P to manure layer
            manwip(j) = manwip(j)+ frt_kg*(fminp(ifrt)+forgp(ifrt))     &
     &      *fwep(ifrt)
            manwop(j) = manwop(j)+ frt_kg*(fminp(ifrt)+forgp(ifrt))*0.05
            mansip(j) = mansip(j)+ frt_kg*(fminp(ifrt)+forgp(ifrt))*    &
     &       (1.-fwep(ifrt)-0.05)*0.25
            mansop(j) = mansop(j)+ frt_kg*(fminp(ifrt)+forgp(ifrt))*    &
