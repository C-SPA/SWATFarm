...
!!    sa            | %           |% sand in soil layer 1
!!    si            | %           |% silt in soil layer 1
!!    cl            | %           |% clay in soil layer 1
!!    ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
...
!! phosphorus changes 2/4/2015 gsm per mjw
	real :: mgt3,mgt4,mgt5,mgt6,mgt7,mgt8,mgt9,sa,cl,si   
...
!! phosphorus changes 2/4/2015 gsm per mjw		     
		     ! estimate VFS_Ks from soil texture by Cibin 08/07/2012 MW 11/29/12
	       sa=sol_sand(1,ihru)
	       cl=sol_clay(1,ihru)
	       si=sol_silt(1,ihru)
	    if (sa>=85) then 
		   VFS_ks(ihru) = 235.44
	     elseif (sa >=70 .AND. sa<=85 .AND. cl<=15 .AND. si<=30) then
		       VFS_ks(ihru)=59.76
	     elseif (sa>=42.5 .AND. sa<=85 .AND. cl<=20 .AND. si<=50) then
		       VFS_ks(ihru)=21.82
	     elseif (sa<=50 .AND. cl<=27.5 .AND. si>=50) then
		       VFS_ks(ihru)= 6.80
	     elseif (sa>=22.5 .AND. sa<=52.5 .AND. cl>=7.25 .AND. cl<=27.5   &
     &       .AND.si>=27.5 .AND. si<=50) then
		       VFS_ks(ihru)=13.21
	     elseif (sa>=45 .AND. sa<=80 .AND. cl>=20 .AND. cl<=35 .AND.     &
     &	     si<=27.5) then
		       VFS_ks(ihru)=3.0
	     elseif (sa<=20 .AND. cl>=27.5 .AND. cl<=40 .AND. si>=40 .AND.   &
     &	     si<=72.5) then
		       VFS_ks(ihru)=2.0
	     elseif (sa<=10 .AND. cl>=40 .AND. cl<=60 .AND. si>=40 .AND.     &
     &	     si<=60) then
		       VFS_ks(ihru)=1.0
	     elseif (sa>=20 .AND. sa<=45 .AND. cl>=27.5 .AND. cl<=40 .AND.   &
     &	     si>=15 .AND. si<=52.5) then
		       VFS_ks(ihru)=2.0
	     elseif (sa>=45 .AND. sa<=65 .AND. cl>=35 .AND. cl<=55 .AND.     &
     &	     si<=20) then
		       VFS_ks(ihru)= 1.2
	       else
		       VFS_ks(ihru) = 0.6
...
!! phosphorus changes 2/4/2015 gsm per mjw commented section below
!! Set defaults
...
!! phosphorus changes 2/4/2015 gsm per mjw commented statement below
