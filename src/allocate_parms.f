...
!     changed for jennifer b
      msubo = 36
	  ! six varibales for SWAT_FARM included in std.out file
      !mstdo = 113
	  mstdo = 132
...
!!    carbon outputs for .hru file
	  ...
	  !! nitrious oxide output for .hru file !! added by Moges Berbero
      allocate(nitr_oxide(mhru))
      allocate(nitr(mhru)) 
      allocate(n2_den(mhru))
	  allocate(sol_org_n(mhru))
	  allocate(sol_N_min(mhru))
	  allocate(sol_P_min(mhru))
!!    carbon outputs for .hru file
...
!!    grazings per year, and HRUs
      ...
      allocate (num_calf(mhru))
      allocate (num_HFD(mhru))
      allocate (num_HSD(mhru))
      allocate (num_HTD(mhru))
      allocate (num_dry(mhru))
...
!!    arrays
   ...
   !   allocate (hhprecip(24))
      ...
      allocate (fcstaao(16))
!! phosphorus changes 2/4/2015 gsm per mjw didn't put the statement below in bc dev code changed to 16
...
!! phosphorus changes 2/4/2015 gsm per mjw looks like dtp_parm chgd to dtp_lwratio in dev code
...
!! phosphorus changes 2/4/2015 gsm per mjw
      ...
      !! Manure layer MW 11/14/12
      ! manure layer
      allocate(manwop(mhru))
      allocate(manwip(mhru))
      allocate(mansop(mhru))
      allocate(mansip(mhru))
      allocate(man_dry(mhru))
      allocate(man_cov(mhru))
      ! fertilizer database
      allocate (ftype(mfdb))
      allocate (fwep(mfdb))
      	  
      !! Instream P Model MW 11/28/12
      allocate(epc_lag(mrch))
      allocate(sedp_benthos(mrch))
   	        	    
   	 !!  VSF update MW 11/29/12
      allocate(VFS_ks(mhru))
      ...
      call zeroini
      call zero_urbn
   !! Added for the SWAT-FARM
      !allocate(Man_hru(mhru))
       !nitr_oxide
