...
!!  phosphorus changes 2/4/2015 gsm per mjw  added newcov below
      real :: gc, gc1, swf, frt_t, xx, newcov
      j = 0
      j = ihru
...
!! to initialize continuous fertilization
     ...
     &                 fnh3n(it) * fminn(it)
     !!  phosphorus changes 2/4/2015 gsm per mjw
      !! mjw manure layer 11/15/12
         if(manp_flag == 1 .and. cswat == 0 .and. ftype(it) == 2) then
          ! add applied mineral P to manure layer   
           manwip(j)=manwip(j)+cfrt_kg(j)*(fminp(it)+forgp(it))*fwep(it)
           manwop(j)= manwop(j)+ cfrt_kg(j)*(fminp(it)+forgp(it))*0.05
           mansip(j)= mansip(j)+ cfrt_kg(j)*(fminp(it)+forgp(it))*      &
     &       (1.-fwep(it)-0.05)*0.25
           mansop(j)= mansop(j)+ cfrt_kg(j)*(fminp(it)+forgp(it))*      &
     &       (1.-fwep(it)-0.05)*0.75
            man_dry(j) = man_dry(j) + cfrt_kg(j) 
           ! update fractional coverage grazing applied
           newcov = (1-man_cov(j))* 0.000025 * cfrt_kg(j)  
     ...
     &                 forgp(it)
!!  phosphorus changes 2/4/2015 gsm per mjw  added end if below
          end if
          end if
