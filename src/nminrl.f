...
!!   mansop_decay |kg P/ha       |amount of phosphorus decayed from stable organic manure on soil surface
!!   mansip_decay |kg P/ha       |amount of phosphorus decayed from stable inorganic manure on soil surface
...
!!  phosphorus changes 2/4/2015 gsm per mjw
      real :: mansop_decay,mansip_decay,manwop_decay,manwip_decay,vap
...
!!  phosphorus changes 2/4/2015 gsm per mjw
          ...
          !! MJW Manure Layer 11/15/12 Decay of stable surface manure P into water extractable forms
           if(manp_flag == 1 .and. k == 1 .and. mansop(j) > 0.001 )then
                ! Decay stable organic manure P on surface in step with residues in layer 1
                mansop_decay = mansop(j)* decr            
 		        mansip_decay = mansip(j)* decr
		        manwop_decay = manwop(j)* decr
		        manwip_decay = manwip(j)* decr             
      	        mansop(j) = mansop(j)- mansop_decay
  		        mansip(j) = mansip(j)- mansip_decay
		        manwop(j) = manwop(j)- manwop_decay
		        manwip(j) = manwip(j)- manwip_decay 
		        
		    !! decayed manure P 60% goes to soil layer 1 40% to layer 2
		       vap = mansop_decay + mansip_decay + manwop_decay + manwip_decay
    		       sol_solp(1,j) = sol_solp(1,j) + vap * 0.6
   	         sol_solp(2,j) = sol_solp(2,j) + vap * 0.4

 		    !! Mineralize stable manure P on surface in step with residues in layer 1
               mansop_decay = mansop(j)* decr * 0.01            
 		       mansip_decay = mansip(j)* decr * 0.0025
		       mansop(j) = mansop(j)- mansop_decay
  		       mansip(j) = mansip(j)- mansip_decay	
		       manwop(j) = manwop(j)+ mansop_decay * 0.25
               manwip(j) = manwip(j)+ mansop_decay * 0.75
!		       manwip(j) = manwip(j)+ mansip_decay 

                ! Decay of dry manure in step with residue in layer 1
                man_dry(j) = man_dry(j)* (1-decr) 
                
                ! recalculate fractional coverage allow linear decay               
                  !! Update manure coverage                
                man_cov(j) = man_cov(j) * (1-decr)
                man_cov(j) = min (man_cov(j), 1.)            
                            
                ! Limits
                if (mansop(j)< 1.e-6) mansop(j)= 0.
                if (mansip(j)< 1.e-6) mansip(j)= 0.
                if (man_dry(j)< 1.e-6) man_dry(j)= 0.
                if (man_cov(j)< 1.e-6) man_cov(j)= 0.
            end if
