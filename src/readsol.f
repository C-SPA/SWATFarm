...
!    change below double subscripted sol_ec statement 1/27/09 when making septic changes
      ...
      !! MJW added rev 490  !!!tlv -- these appear to be in switched order from .sol file
	!!CaCo3 content (%) 
	if (eof < 0) exit	
...
!            sol_k(j,ihru) = c
        ...
        if (sol_awc(j,ihru) <= 0.) sol_awc(j,ihru) = .005  !!tlv Armen already placed limits above on this
