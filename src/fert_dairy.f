...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j, l, ifrt
!!  phosphorus changes 2/4/2015 gsm per mjw  added newcov 
      real :: xx, gc, gc1, swf, frt_t, newcov
      ...
      ifrt = 0
      ifrt = ifrttyp
      !fminn(ifrt) = Nmin_frac
      !forgn(ifrt) = Norg_frac
	  !fminp(ifrt) = Pmin_frac
	  !forgp(ifrt) = Porg_frac 
...
!!  phosphorus changes 2/4/2015 gsm per mjw
     ...
     &      frt_kg * Norg_frac
            ...
            !! MJW Manure P Layer
           if(manp_flag == 1 .and. l == 1 .and. Porg_frac > 0. .and.    &
     &      ftype(ifrt) == 2) then
          ! add applied mineral P to manure layer
            manwip(j) = manwip(j)+ frt_kg*(Pmin_frac+Porg_frac)         &
     &      *fwep(ifrt)
            manwop(j) = manwop(j)+ frt_kg*(Pmin_frac+Porg_frac)*0.05
            mansip(j) = mansip(j)+ frt_kg*(Pmin_frac+Porg_frac)*        
     &       (1.-fwep(ifrt)-0.05)*0.25
            mansop(j) = mansop(j)+ frt_kg*(Pmin_frac+Porg_frac)*        &
     &       (1.-fwep(ifrt)-0.05)*0.75
            man_dry(j) = man_dry(j) + frt_kg
            
           ! update fractional coverage machine applied
           newcov =(1-man_cov(j))* 0.012 * frt_kg ** 0.48
           man_cov(j) = man_cov(j)+ newcov
           man_cov(j) = min (man_cov(j), 1.)                  
...
!!  phosphorus changes 2/4/2015 gsm per mjw  commented lines below
!!        sol_fon(l,j) = sol_fon(l,j) + rtof * xx *                       
!!     &     frt_kg                    * Norg_frac
!!	  sol_aorgn(l,j) = sol_aorgn(l,j) + (1. - rtof) * xx *            
!!     &     frt_kg                    * Norg_frac
...
!!     &      frt_kg                    * Porg_frac
     ...
     &		Porg_frac
	  ...
	  if (cswat == 2) then
        !sol_fon(l,j) = sol_fon(l,j) + rtof * xx *                       &
     &  !   frt_kg(nro(j),nfert(j),j) * Norg_frac
	  !sol_aorgn(l,j) = sol_aorgn(l,j) + (1. - rtof) * xx *            
     &  !   frt_kg(nro(j),nfert(j),j) * Norg_frac
        sol_fop(l,j) = sol_fop(l,j) + rtof * xx *        
     &      frt_kg * Porg_frac
     ...
     &                  frt_kg * Norg_frac
          ...
          RLN = .175 *(orgc_f)/(Nmin_frac + Norg_frac + 1.e-5)
...
!! summary calculations
      fertno3 = frt_kg * Nmin_frac * (1. - fnh3n(ifrt))
      fertnh3 = frt_kg * (Nmin_frac * fnh3n(ifrt))
      fertorgn = frt_kg * Norg_frac
      fertsolp = frt_kg * Pmin_frac
      fertorgp = frt_kg * Porg_frac  
     ...
     &   Nmin_frac * (1. - fnh3n(ifrt))

      wshd_fnh3 = wshd_fnh3 + frt_kg                    * hru_dafr(j) * 
     &   Nmin_frac * fnh3n(ifrt)
