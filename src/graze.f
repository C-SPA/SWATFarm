...
!!  phosphorus changes 2/4/2015 gsm per mjw
           ...
           if(manp_flag == 1) then
            ...
            manwip(j) = manwip(j)+ manure_kg(j)*(fminp(it)+forgp(it))*  &
     &      fwep(it)
            manwop(j) = manwop(j)+ manure_kg(j)*(fminp(it)+forgp(it))*  &
     &      0.05
            mansip(j) = mansip(j)+ manure_kg(j)*(fminp(it)+forgp(it))*  &
     &      (1-fwep(it)-0.05)*0.25
            mansop(j) = mansop(j)+ manure_kg(j)*(fminp(it)+forgp(it))*  &
     &      (1-fwep(it)-0.05)*0.75
   
            man_dry(j) =  man_dry(j) + manure_kg(j)        
            ! recalculate cover grazing deposition
            newcov = (1-man_cov(j))* 0.000025 * manure_kg(j)
