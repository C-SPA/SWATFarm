...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j, ly, ifrt
      real :: tsno3, tpno3, dwfert, xx, targn, tfp, yy
...
!! add nutrients to soil based on nitrogen need
     ...
     &                    * dwfert * forgn(ifrt)  
              ...
              !! put P organics in surface manure MJW 11/15/2012
            if (manp_flag==1 .and. ly==1 .and. ftype(ifrt)==2)then
                mansop(j) = mansop(j)+ rtoaf * xx * dwfert *            &
     &         (fminp(ifrt)+forgp(ifrt))*(1-fwep(ifrt)-0.05)*0.75
                manwop(j) = manwop(j)+ rtoaf * xx * dwfert *            &
     &         (fminp(ifrt)+forgp(ifrt))*0.05
                man_dry(j) = man_dry(j) + dwfert
...
!!            sol_fop(ly,j) = sol_fop(ly,j) + rtoaf * xx * dwfert         
     ...
     &                      *forgn(ifrt) -ZZ
	    ...
	    !!=================
          ...
          !! MJW 11/15/12 for surface manures
          if (manp_flag == 1 .and. ly == 1 .and. ftype(ifrt) == 2) then
          ! add applied mineral P to manure layer
                    ! add applied mineral P to manure layer         
            mansip(j) = mansip(j)+ rtoaf * xx * dwfert *                &
     &     (tfp+forgp(ifrt))*(1-fwep(ifrt)-0.05)*0.25
            manwip(j) = manwip(j)+ rtoaf * xx * dwfert *                &
     &     (tfp+forgp(ifrt))*fwep(ifrt)
           else ! use existing code
