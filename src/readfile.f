...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      !!tlv added to specify outputTV.snu header more efficiently
      integer, parameter :: hdrSize = 73
      character(len=11), dimension(1:hdrSize) :: hdrString 
...
!!tlv March 25, 2016. solp.out file moved to readbsn.f after isolp read. no need to open file if not needed.
!!! phosphorus changes 2/4/2015 gsm
!!!   added 11/29/2012 for MJW by gsm       
!      open (177,file="solp.out")
!      write (177,12223)
!12223 format ('year         day         hru       sol_solp          surf
!     &q        man_drv        man_cov       surqsolp     wep_i_leach   w
!     &ep_o_leach        mansolp         manwip         manwop         ma
!     &nsip         mansop')

      open (178,file="instreamP.out")
      write (178,12224)
12224 format ('Year Day Reach      flow      solp-in     solp-out    org&
     &p-in    orgp-out       EPC       benp      solp_benp   orgp_benp  &
     & solp_orgp ')         
...
!!    tlv May 2016, created a variation of output.snu with the mass values for
!!        independent soil layers. Also, switched to csv.
          open (1210, file='output_mass.snu')
               ! SOLN = surqno3, Qtot = WYLD, Qover = surfQ, 
	        hdrString =(/"    HRU_sub", "        SUB",  "         YR",
     &                   "         MO", "         DA",  "        JDY",
     &                   "     AREAha", "    DAILYCN",  "    Qtot_m3",
     &	                 "    Qovr_m3", " EDIflat_m3",  " EDIperc_m3",
     &                   "   EDIsw_m3", "   EDIfc_m3",  "  L1flat_m3",
     &                   "  L1perc_m3", "    L1sw_m3",  "    L1fc_m3",
     &                   "  L2flat_m3", "  L2perc_m3",  "    L2sw_m3",
     &      	         "    L2fc_m3", "  L3flat_m3",  "  L3perc_m3",
     &                   "    L3sw_m3", "    L3fc_m3",  "   SEDYLD_t",
     &                   "    SOLP_kg", "  SEDPac_kg",  "  SEDPst_kg",
     &                   "    ORGP_kg", " EDIsolP_kg",  " EDIorgP_kg",
     &                   "EDIforgP_kg", "  L1solP_kg",  "  L1orgP_kg",
     &                   " L1forgP_kg", "  L2solP_kg",  "  L2orgP_kg",
     &                   " L2forgP_kg", "  L3solP_kg",  "  L3orgP_kg",
     &                   " L3forgP_kg", "    SOLN_kg",  "    ORGN_kg",
     &                   " EDIno3N_kg", " EDIorgN_kg",  " EDInh3N_kg",
     &                   "EDIaorgN_kg", "EDIforgN_kg",  "  L1no3N_kg",
     &                   "  L1orgN_kg", "  L1nh3N_kg",  " L1aorgN_kg",
     &                   " L1forgN_kg", "  L2no3N_kg",  "  L2orgN_kg",
     &                   "  L2nh3N_kg", " L2aorgN_kg",  " L2forgN_kg",
     &                   "  L3no3N_kg", "  L3orgN_kg",  "  L3nh3N_kg",
     &                   " L3aorgN_kg", " L3forgN_kg",  " EDIsoil_Mg",
     &                   "  EDIrsd_Mg", "  L1soil_Mg",  "   L1rsd_Mg",
     &                   "  L2soil_Mg", "   L2rsd_Mg",  "  L3soil_Mg",
     &                   "   L3rsd_Mg"/)
          write (1210, '(A,72(",",A))') trim(hdrString(1)), 
     &             (trim(hdrString(k)), k = 2, hdrSize)
              !tlv -- here EDI is the 10mm SWAT-created soil layer and the
              ! other layer numbers refer to the layers input by the user
...
!  0=no print 1=print
	...
	if (imgt==1) then
         ...
         write (143,999)
999      format(2x,'Sub',4x,'Hru',3x,'Year',3x,'Mon',3x,'Day',
...
! write statement added for Aziz (06/25/09)
 ...
 1000  format (",",'SUB',t6,'HRU',t12,'DAY',t17,'YEAR',t26,'VOL-I',t37,  
     &'SA-I',t46,'SPILLO', 
...
2222  format (3x,'yr',2x,'day',5x,'res_vol',7x,'res_no3',7x,'ressa',
 ...
 1010 format (32x,'SNOW(mm) at ELEVATION BAND (1-10)',/,                
     &",",'DAY','   YR',t14,'GISnum',t28,'1',t36,'2',t44,'3',t52,'4',    
