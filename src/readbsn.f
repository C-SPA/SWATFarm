...
!!                               |3 model stream water quality with modified P instream model watqual3
...
!     iatmodep = 0 - average annual = 1 - monthly  2 = daily
      ...
      if (eof < 0) exit
      read (103,*,iostat=eof) manp_flag !! flag for surface manure laver by Peter Vadas set to 1 to use Manure P model
      if (eof < 0) exit
!!    added check for print out of solp.out gsm 5/4/2015      
      read (103,*,iostat=eof) isolp    !! flag to create solp.out file should be 1 if need to create file  
      if (eof < 0) exit
!!    added for user input values of SWAT-FARM model 	  !FOR DAIRY
      ...
      if (eof < 0) exit	 
      read (103,*,iostat=eof) iHeifer_first_lact   !! number of Heifer_first_lact   
      if (eof < 0) exit	
      read (103,*,iostat=eof) iHeifer_second_lact   !! number of Heifer_second_lact   
      if (eof < 0) exit	
	  read (103,*,iostat=eof) iHeifer_third_lact  !! number of Heifer_third_lact   
      if (eof < 0) exit
	  read (103,*,iostat=eof) iHeifer_first_dry   !! number of Heifer_first_dry    
      if (eof < 0) exit
	  read (103,*,iostat=eof) iHeifer_second_dry   !! number of Heifer_second_dry   
      if (eof < 0) exit	
	  read (103,*,iostat=eof) iHeifer_third_dry   !! number of Heifer_third_dry  
      if (eof < 0) exit	
	  read (103,*,iostat=eof) iLact   !! number of Lact
      if (eof < 0) exit
...
!!    at some future time)
      ...
      write (100,*) 'ly', 'year',';','day',';','hru',';','cmass',';',
     & 'sol_rsd',';','mancmass',';','nmass'
      end if
     ...
     &'WFSC9','WFSC10','WFSC11'
      ...
      !!add by Moges Berbero for SWAT-FARM (11/20/2018)
      !!=====================
      !!=====================
      if (dairy == 1) then
       open (1798,file="dairy.txt", recl = 969)
        write (1798,1222)
	   open (1799,file="Fert_appli.txt", recl = 979)
       write (1799,1232),'nDay','Nmin_frac','fminn','Pmin_frac', 
     & 'fminp'
	  
	   open (1797,file="Fert_Amount.txt", recl = 989)
       write (1797,1242),'nDay','FERT_AMount'
	  !end if
...
!!tlv March 25, 2016. solp.out file moved from readfile.f. no need to open file if not needed.      
...
12223 format ('year         day         hru       sol_solp          surf
 ...
 !!1222  format (a11,11a11)
 1222  format ('nDay',t13,'TotalNP_barn',t28'total_N_barn',t44,
     &'total_P_barn',t60,'total_N_Past',t75,'total_P_Past',t89,
     &'Norg_barn',t105,'Nmin_barn',t120,'Porg_barn',t135,
     &'Pmin_barn',t150,'Norg_past',t165,'Nmin_past',t180,
     &'Porg_past',t195,'Pmin_past',t210,'Pmin_frac',t225,
     &'Porg_frac',t240,'Norg_frac',t255,'Norg_frac' )
 1232 format(5a11)
 1242 format(2a11)
      end
