...
1002  format (a5,1x,a4,3i6,1x,e10.5,1x,2a15,7f10.2,10x,f10.2,70x,2i7)
          ...
          case (3)   !! fertilizer operation
		    ...
		    if (dairy_flag ==1) then
            ifrttyp = mgt1iop(nop(j),j)
            frt_kg = mgt4op(nop(j),j)
            Frt_Man  =  Frt_Man + frt_kg * hru_km(j)*100
             !call dairy_total
             print*, Frt_Man, TotalNP_barn
            if (Frt_Man < TotalNP_barn) then
              !TotalNP_barn = TotalNP_barn
              TotalNP_barn =  TotalNP_barn - Frt_Man

              else
              STOP 'Reduce Fert_kg amount'
              end if
             !Frt_day  =  frt_kg * hru_km(j)*100
...
1004  format (a5,1x,a4,3i6,1x,e10.5,1x,2a15,7f10.2,20x,f10.2,10x,5f10.2)
            ...
            !if (j == mhru) then
		    ! write(1797,9009)i,j,Frt_Man, Frt_day
 !9009  format(2i4,1x,2f15.3)
		     !end if
...
1003  format (a5,1x,a4,3i6,1x,e10.5,1x,2a15,7f10.2,30x,f10.2)
     ...
     *        sol_rsd(1,j),sol_sumno3(j),sol_sumsolp(j)
          ...
          case (9)    !! grazing operation
            ...
            num_calf(j) = mgt5op(nop(j),j) ! Number of calf
			 HPN_calf =(num_calf(j)*wtCalf*0.04*grz_days(j))/bio_min(j) ! hectares of pasture needed for calf
             print*,HPN_calf,hru_km(j)*100
             if (HPN_calf > hru_km(j)*100) then
                 print*,'Reduce num_calf number'
			  ndeat(j) = 0
              igrz(j) = 1	
              end if
             if ((num_calf(j)*(N_calf+P_calf)) < = 0.) then 
                 num_calf(j) = 0.95* mgt4op(nop(j),j)/(N_calf+P_calf)
                 end if
            num_HFD(j) =mgt6op(nop(j),j) ! number of heifer fisrt dry
             HPN_HFD=(num_HFD(j)*WTHFD*0.04*grz_days(j))/bio_min(j) ! hectares of pasture needed for heifer first dry
             if (HPN_HFD > hru_km(j)*100) then
                 print*,'Reduce num_HFD number'
              ndeat(j) = 0
              igrz(j) = 1
			  end if
             if ((num_HFD(j)*NP_HFD) < = 0.) then 
                 num_HFD(j) = 0.95* mgt4op(nop(j),j)/(NP_HFD)
                 end if			  
            num_HSD(j) = mgt7op(nop(j),j)
             HPN_HSD=(num_HSD(j)*WTSFD*0.04*grz_days(j))/bio_min(j) ! hectares of pasture needed for heifer 2nd dry
             if (HPN_HSD > hru_km(j)*100) then
                 print*,'Reduce num_HSD number'
              ndeat(j) = 0
              igrz(j) = 1
			  end if
              if ((num_HSD(j)*NP_HSD) < = 0.) then 
                 num_HSD(j) = 0.95* mgt4op(nop(j),j)/(NP_HSD)
                 end if
            num_HTD(j) = mgt8op(nop(j),j)
             HPN_HTD =(num_HTD(j)*WTHTD*0.04*grz_days(j))/bio_min(j) ! hectares of pasture needed for heifer 3rd dry
             if (HPN_HTD > hru_km(j)*100) then
                 print*,'Reduce num_HTD number'
             ndeat(j) = 0
             igrz(j) = 1
             end if
             if ((num_HTD(j)*NP_HTD) < = 0.) then 
                 num_HTD(j) = 0.95* mgt4op(nop(j),j)/(NP_HTD)
                 end if
            num_dry(j) = mgt9op(nop(j),j)
             HPN_dry =(num_dry(j)*wtDry*0.04*grz_days(j))/bio_min(j) ! hectares of pasture needed for heifer dry
             if (HPN_dry > hru_km(j)*100) then
                 print*,'Reduce num_dry number'
             ndeat(j) = 0
             igrz(j) = 1
             end if
             if ((num_dry(j)*(N_dry+P_dry)) < = 0.) then 
                 num_dry(j) = 0.95* mgt4op(nop(j),j)/(N_dry+P_dry)
                 end if				 
	         call graze_dairy			  
		    end if
     ...
     *        sol_rsd(1,j),sol_sumno3(j),sol_sumsolp(j)
      ...
      if (nop(j) > nopmx(j)) then
        nop(j) = 1
           ...
           ! if (j == mhru) then
		    ! write(1797,9010)"Tot",i,Frt_Man
!9010   !format(a4,i4,1x,f15.3)
             !end if
