...
!!  phosphorus changes 2/4/2015 gsm per mjw
!!    manwop        |kg/ha         | manure P on surface that is Organic Water extractable
!!    manwip        |kg/ha         | manure P on surface that is Mineral Water extractable P 
!!    mansip        |kg/ha         | manure P on surface that is Mineral Stable P 
!!    mansop        |kg/ha         | manure P on surface that is Organic Stable P
!!    man_dry       |kg/ha         | dry manure on surface
!!    man_cov       |fractional    | fraction of surface covered by manure
!!    manp_flag     |flag          | flag for manure P surface layer by Vadas 2007
...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j, l, it
!!  phosphorus changes 2/4/2015 gsm per mjw   added newcov below
      real :: dmi, dmii, gc, gc1, swf, frt_t, xx, newcov
...
!! graze only if adequate biomass in HRU
     ...
     &                         (0.42 * LSF * resnew / 150) + 1.E-25
		...
		!fminp(it) = PminP_frac
		!forgp(it) = PorgP_frac
        manure_kg(j) = (num_calf(j)*NP_calf)+(num_HFD(j)*NP_HFD)+
     &  (num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+ (num_dry(j)*NP_dry)
		
        if ((num_calf(j)*NP_calf)+(num_HFD(j)*NP_HFD)+(num_HSD(j)*
     &    NP_HSD)+(num_HTD(j)*NP_HTD)+(num_dry(j)*NP_dry)> 0.) then 
          l = 1
...
!!  phosphorus changes 2/4/2015 gsm per mjw
           !! MJW Manure P Layer
           if(manp_flag == 1) then
            ! add applied mineral P to manure layer
            manwip(j) = manwip(j)+ ((num_calf(j)*NP_calf)+(num_HFD(j)*
     &                NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &      (num_dry(j)*NP_dry))*(PminP_frac+PorgP_frac)*fwep(it)
            manwop(j) = manwop(j)+ ((num_calf(j)*NP_calf)+(num_HFD(j)*
     &                NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &      (num_dry(j)*NP_dry))*(PminP_frac+PorgP_frac)*0.05
            mansip(j) = mansip(j)+ ((num_calf(j)*NP_calf)+(num_HFD(j)*
     &                NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &      (num_dry(j)*NP_dry))*(PminP_frac+PorgP_frac)*(1-fwep(it)-
     &                0.05)*0.25
            mansop(j) = mansop(j)+ ((num_calf(j)*NP_calf)+(num_HFD(j)*
     &                NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &      (num_dry(j)*NP_dry))*(PminP_frac+PorgP_frac)*(1-fwep(it)-
     &                0.05)*0.75 
            man_dry(j) = man_dry(j)+((num_calf(j)*NP_calf)+(num_HFD(j)*
     &                 NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &     (num_dry(j)*NP_dry))        
            ! recalculate cover grazing deposition
            newcov = (1-man_cov(j))* 0.000025 *(num_calf(j)*NP_calf)+
     &     (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &     (num_dry(j)*NP_dry)
            man_cov(j) = man_cov(j)+ newcov
               
          else ! use existing code
            sol_no3(l,j) = sol_no3(l,j) + (num_calf(j)*NP_calf)+
     &      (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &     (num_dry(j)*NP_dry)*(1. - fnh3n(it))*NminP_frac
          sol_fon(l,j) = sol_fon(l,j) + (num_calf(j)*NP_calf)+
     &     (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &    (num_dry(j)*NP_dry)*NorgP_frac
          sol_nh3(l,j) = sol_nh3(l,j) + ((num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &    (num_dry(j)*NP_dry))*fnh3n(it)*NminP_frac
          sol_solp(l,j) = sol_solp(l,j) + (num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &    (num_dry(j)*NP_dry)*PminP_frac
          sol_fop(l,j) = sol_fop(l,j) + (num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &    (num_dry(j)*NP_dry)*PorgP_frac
          end if
!!  phosphorus changes 2/4/2015 gsm per mjw
         end if
          if (cswat == 1) then
     ...
     &     (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &     (num_dry(j)*NP_dry))*(1. - fnh3n(it))*NminP_frac
          sol_mn(l,j) = sol_mn(l,j) + ((num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &    (num_dry(j)*NP_dry))*NorgP_frac
          sol_nh3(l,j) = sol_nh3(l,j) + ((num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD))+
     ...
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &    (num_dry(j)*NP_dry))*PminP_frac
          sol_mp(l,j) = sol_mp(l,j) + (num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &     (num_dry(j)*NP_dry)*PorgP_frac          
          sol_mc(l,j) = sol_mc(l,j) + (num_calf(j)*NP_calf)+
     &     (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &     (num_dry(j)*NP_dry)*NorgP_frac * 10.
          end if
     ...
     &    !             NorgP_frac
          orgc_f = 0.35  
          X1 = (num_calf(j)*NP_calf)+(num_HFD(j)*NP_HFD)+(num_HSD(j)*
     &        NP_HSD)+(num_HTD(j)*NP_HTD)+(num_dry(j)*NP_dry)
          X8 = X1 * orgc_f          
          RLN = .175 *(orgc_f)/(PminP_frac + NorgP_frac + 1.e-5)
          X10 = .85-.018*RLN
          if (X10<0.01) then
            X10 = 0.01
          else
          ...
          YY = ((num_calf(j)*NP_calf)+(num_HFD(j)*NP_HFD)+(num_HSD(j)*
     &       NP_HSD)+(num_HTD(j)*NP_HTD)+(num_dry(j)*NP_dry)) * X10
          sol_LM(l,j) = sol_LM(l,j) + YY
          ZZ = ((num_calf(j)*NP_calf)+(num_HFD(j)*NP_HFD)+(num_HSD(j)*
     &       NP_HSD)+(num_HTD(j)*NP_HTD)+(num_dry(j)*NP_dry)) *NorgP_frac * X10
          sol_LMN(l,j) = sol_LMN(l,j) + ZZ
          sol_LSN(l,j) = sol_LSN(l,j) + ((num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &    (num_dry(j)*NP_dry))*NorgP_frac -ZZ
          XZ = ((num_calf(j)*NP_calf)+(num_HFD(j)*NP_HFD)+(num_HSD(j)*
     &        NP_HSD) +(num_HTD(j)*NP_HTD)+(num_dry(j)*NP_dry))*orgc_f-XX
          sol_LSC(l,j) = sol_LSC(l,j) + XZ
          sol_LSLC(l,j) = sol_LSLC(l,j) + XZ * .175          
          sol_LSLNC(l,j) = sol_LSLNC(l,j) + XZ * (1.-.175) 
          YZ = ((num_calf(j)*NP_calf)+(num_HFD(j)*NP_HFD)+(num_HSD(j)*
     &       NP_HSD)+(num_HTD(j)*NP_HTD)+(num_dry(j)*NP_dry)) - YY
...
!! calculate ground cover
          ...
          frt_t = 0.
          frt_t = bact_swf * ((num_calf(j)*NP_calf)+(num_HFD(j)*
     &           NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &    (num_dry(j)*NP_dry))/ 1000.
        ...
        grazn = grazn + (num_calf(j)*NP_calf)+(num_HFD(j)*NP_HFD)+
     &  (num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+(num_dry(j)*NP_dry) *
     &        (NminP_frac + NorgP_frac)
        grazp = grazp + (num_calf(j)*NP_calf)+(num_HFD(j)*NP_HFD)+
     &  (num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+(num_dry(j)*NP_dry) *
     &        (PminP_frac + PorgP_frac)
        tgrazn(j) = tgrazn(j) + grazn
        tgrazp(j) = tgrazp(j) + grazp

        if (curyr > nyskip) then
          wshd_ftotn = wshd_ftotn + ((num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &    (num_dry(j)*NP_dry))*hru_dafr(j)*(NminP_frac+ NorgP_frac)
          wshd_forgn = wshd_forgn + ((num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &   (num_dry(j)*NP_dry))*hru_dafr(j)*NorgP_frac
          wshd_fno3 = wshd_fno3+((num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &   (num_dry(j)*NP_dry))*hru_dafr(j)*NminP_frac*(1. - fnh3n(it))
          wshd_fnh3 = wshd_fnh3+((num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &    (num_dry(j)*NP_dry))*hru_dafr(j)*NminP_frac * fnh3n(it)
          wshd_ftotp = wshd_ftotp+((num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &    (num_dry(j)*NP_dry))*hru_dafr(j)*(PminP_frac + PorgP_frac)
          wshd_fminp = wshd_fminp+((num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &    (num_dry(j)*NP_dry))*hru_dafr(j)*PminP_frac
          wshd_forgp = wshd_forgp+((num_calf(j)*NP_calf)+
     &    (num_HFD(j)*NP_HFD)+(num_HSD(j)*NP_HSD)+(num_HTD(j)*NP_HTD)+
     &    (num_dry(j)*NP_dry))*hru_dafr(j)*PorgP_frac
